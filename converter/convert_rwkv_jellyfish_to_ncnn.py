...
for i in range(n_layer):
    if version == 6:
        layer_count, blob_count = build_time_mixing_v6(ncnn_param_lines, ncnn_weights_file, weights, layer_input, f'time_mixing_{i}_out', i, layer_count, blob_count, n_head, head_size, jellyfish_modified=True)
        layer_count, blob_count = build_channel_mixing_v6(ncnn_param_lines, ncnn_weights_file, weights, f'time_mixing_{i}_out', f'channel_mixing_{i}_out', i, layer_count, blob_count)
    elif version == 7:
        layer_count, blob_count = build_time_mixing_v7(ncnn_param_lines, ncnn_weights_file, weights, layer_input, f'time_mixing_{i}_out', i, layer_count, blob_count, n_head, head_size, jellyfish_modified=True)
        layer_count, blob_count = build_channel_mixing_v7(ncnn_param_lines, ncnn_weights_file, weights, f'time_mixing_{i}_out', f'channel_mixing_{i}_out', i, layer_count, blob_count)
    else:
